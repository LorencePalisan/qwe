...
import { FREELANCE_ENQUIRIES, type FreelanceEnquiry } from "@/lib/mock/freelance";
...
type StatusTab = "all" | FreelanceEnquiry["status"];
...
function ReplyModal({
  enquiry, onSend, onClose,
}: { enquiry: FreelanceEnquiry; onSend: (id: string, reply: string) => void; onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 backdrop-blur-sm sm:items-center sm:p-4">
      <div className="w-full max-w-lg rounded-t-2xl border border-border bg-card p-5 shadow-2xl sm:rounded-2xl">
        <div className="mb-4 flex items-start justify-between gap-3">
          ...
          <button onClick={onClose} className="shrink-0 rounded-lg p-1 text-foreground/60 hover:bg-foreground/8"><X className="size-4" /></button>
        </div>
        ...
        <div className="mt-3 flex gap-3">
          <button onClick={onClose} className="flex-1 rounded-xl border border-border py-2.5 text-[13px] font-medium text-foreground/60 hover:bg-foreground/5">Cancel</button>
          <button
            onClick={() => { if (text.trim()) { onSend(enquiry.id, text.trim()); onClose(); } }}
            disabled={!text.trim()}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#F62C7D] py-2.5 text-[13px] font-semibold text-white hover:opacity-90 disabled:opacity-40"
          >
...
function InquiryCard({
  enq, onMarkRead, onReply, onClose,
}: { enq: FreelanceEnquiry; onMarkRead: (id: string) => void; onReply: (e: FreelanceEnquiry) => void; onClose: (id: string) => void }) {
  ...
  return (
    <div className={cn("border-b border-border last:border-b-0", expanded && "bg-foreground/2")}>
      <button onClick={toggle} className="flex w-full items-start gap-3 px-4 py-4 text-left hover:bg-foreground/3 transition-colors">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <p className={cn("text-[13px] text-foreground", enq.status === "pending" ? "font-bold" : "font-semibold")}>{enq.subject}</p>
            <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-semibold", stCfg.className)}>{stCfg.label}</span>
          </div>
          <p className="text-[11px] text-foreground/50">{enq.fromName}</p>
        </div>
        <div className="shrink-0 text-right">
          <p className="text-[11px] text-foreground/60">{new Date(enq.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</p>
          {expanded ? <ChevronUp className="ml-auto mt-1 size-3.5 text-foreground/60" /> : <ChevronDown className="ml-auto mt-1 size-3.5 text-foreground/60" />}
        </div>
      </button>
        ...
        <div className="px-4 pb-4">
          ...
          {enq.reply && (
            <div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-800 dark:bg-emerald-900/20">
              ...
              {enq.repliedAt && (
                <p className="mt-1.5 text-[11px] text-foreground/55">{new Date(enq.repliedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</p>
              )}
            </div>
          )}
          {enq.status !== "closed" && (
            <div className="mt-3 flex gap-2">
              {enq.status !== "replied" && (
                <button onClick={() => onReply(enq)} className="flex items-center gap-1.5 rounded-lg bg-[#F62C7D]/10 px-3 py-2 text-[12px] font-medium text-[#F62C7D] hover:opacity-80">
                  <Send className="size-3" /> Reply
                </button>
              )}
              <button onClick={() => onClose(enq.id)} className="rounded-lg border border-border px-3 py-2 text-[12px] font-medium text-foreground/50 hover:bg-foreground/5">
                Close inquiry
              </button>
            </div>
          )}
...
export default function FreelanceInquiries() {
  const [enquiries, setEnquiries]       = useState<FreelanceEnquiry[]>(FREELANCE_ENQUIRIES);
  const [tab, setTab]                   = useState<StatusTab>("all");
  const [replyTarget, setReplyTarget]   = useState<FreelanceEnquiry | null>(null);

  function handleMarkRead(id: string) {
    setEnquiries((prev) => prev.map((e) => e.id === id && e.status === "pending" ? { ...e, status: "read" } : e));
  }
  function handleReply(id: string, reply: string) {
    setEnquiries((prev) => prev.map((e) => e.id === id ? { ...e, status: "replied", reply, repliedAt: new Date().toISOString() } : e));
  }
  ...
  return (
    <>
      {replyTarget && <ReplyModal enquiry={replyTarget} onSend={handleReply} onClose={() => setReplyTarget(null)} />}
      ...
      <div className="pb-16">
        <div className="border-b border-border bg-card/50 px-6 py-5">
          <div className="flex items-center gap-2">
            ...
            {pendingCount > 0 && (
              <span className="flex size-5 items-center justify-center rounded-full bg-[#F62C7D] text-[10px] font-bold text-white">{pendingCount}</span>
            )}
          </div>
        ...
        <div className="px-4 py-6 sm:px-6">
          <div className="mb-4 flex gap-1.5 overflow-x-auto pb-0.5 scrollbar-styled">
            {TABS.map(({ value, label }) => {
              ...
              return (
                ...
                >
                  {label}
                  <span className={cn("rounded-full px-1.5 py-0.5 text-[10px] font-bold", tab === value ? "bg-[#F62C7D]/15 text-[#F62C7D]" : "bg-foreground/8 text-foreground/60")}>{count}</span>
                </button>
              );
            })}
          </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card">
            {filtered.length === 0
              ...
              : filtered.map((enq) => (
                <InquiryCard key={enq.id} enq={enq} onMarkRead={handleMarkRead} onReply={setReplyTarget} onClose={handleClose} />
              ))
            }
          </div>
        </div>
