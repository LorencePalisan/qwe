...
function OrganizerLayoutInner() {
  ...
  useEffect(() => {
    ...
    if (!raw) { navigate(`/?redirect=${encodeURIComponent(location.pathname)}`, { replace: true }); return; }
    try {
  ...
  return (
    <div className={cn(theme, "min-h-screen bg-background font-sans")}>
      ...
      <main className={cn("min-h-screen pt-16 xl:pt-0 transform-gpu transition-[padding-left] duration-200 ease-in-out", sidebarOpen ? "xl:pl-64" : "xl:pl-[72px]")}>
        <Outlet />
      </main>
    </div>
  );
...
export default function OrganizerLayout() {
  return <ThemeProvider><OrganizerLayoutInner /></ThemeProvider>;
}
